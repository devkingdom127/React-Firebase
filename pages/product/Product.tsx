  ...
  useEffect(() => {
    ...
    const getProduct = async () => {
      if (location.state && location.state.hasOwnProperty("product")) {
        ...
        const productGet = await axios(
          `${process.env.NEXT_PUBLIC_API_ENDPOINT}/products/popularById/${productid}`
        );
    ...
    const getStores = async () => {
      const stores = await axios(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/store/byProductId/${productid}`,
        {
          params: coords,
        }
      );
    ...
    if (product && product.brand) {
      const getPopularBrand = async () => {
        const result = await axios(
          `${process.env.NEXT_PUBLIC_API_ENDPOINT}/brands/popular/${product.brand}`
        );
