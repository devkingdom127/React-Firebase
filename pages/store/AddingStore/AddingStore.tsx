...
export function AddingStore() {
  ...
  useEffect(() => {
    (async () => {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/register/plans`);
      setPlans(res.data);
      setSeletedPlan(res.data.find((p: any) => p.id === "basic"));
      // setPlans(await axios.get(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/register/plans`));
    })();
  }, []);
  ...
  async function setStoreAlreadyExisting(placeId: string) {
    if (placeId) {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/store/get/${placeId}`
      );
