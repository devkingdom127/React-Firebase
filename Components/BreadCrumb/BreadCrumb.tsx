    ...
    return (
        <nav>
            <ul className='breadcrumb'>
                <li key="home" className='breadcrumb-item'> <Link href='/'> <FontAwesomeIcon icon={faHome} /> {i18n.t('home')} </Link> </li>
                {Arrow()}

                {
                    product ?
                    <>
                        <li key="search" className='breadcrumb-item'> <Link href={`/home?brand=${brand || ''}&product=${product}`}> <FontAwesomeIcon icon={faSearch} /> {i18n.t("search.label")} </Link> </li>
                        {Arrow()}
                    </> : <></>
                }
                    ...
                    return (
                        <React.Fragment key={index}>
                            <li title={item.label} className={`breadcrumb-item ${item.isActive ? 'active' : ''}`}>
                                {item.isActive ? item.label : <Link href={item.route || '/'}> {item.label} </Link>}
                            </li>
